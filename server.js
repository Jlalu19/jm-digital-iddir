...
<head>
    ...
    <style>
        :root { --bg-main: #0b0f19; --bg-card: #141b2d; --accent: #f59e0b; --accent-hover: #d97706; }
        body { background-color: var(--bg-main); color: #fff; font-family: 'Segoe UI', sans-serif; }
        .card-custom { background-color: var(--bg-card); border: 1px solid #1f293d; border-radius: 15px; padding: 25px; margin-bottom: 25px; box-shadow: 0 8px 24px rgba(0,0,0,0.2); }
        .form-control-custom { background-color: #1f293d; border: 1px solid #334155; color: white; border-radius: 8px; padding: 10px; margin-bottom: 12px; }
        .form-control-custom:focus { background-color: #1f293d; color: white; border-color: var(--accent); box-shadow: none; }
        ...
        .btn-premium:hover { transform: translateY(-1px); color: #000; }
        .culture-section { background: linear-gradient(135deg, #1e1b4b 0%, #11103c 100%); border-left: 5px solid var(--accent); padding: 20px; border-radius: 12px; margin-bottom: 25px; }
        ...
        .table-custom th { background-color: #1f293d !important; color: var(--accent) !important; }
        .table-custom td { background-color: var(--bg-card) !important; color: white !important; border-color: #1f293d; }
    </style>
</head>
<body>
        ...
        <div id="dashboard-box" style="display:none;">
            ...
            <div id="superadmin-section" style="display:none;">
                ...
                <div class="card-custom">
                    ...
                    <h5 class="text-warning fw-bold mt-4 mb-2"><i class="fa-solid fa-list"></i> የተመዘገቡ እድሮች አጠቃላይ ዝርዝር</h5>
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="card-custom text-center">
                            ...
                            <p class="small text-muted mb-0">የፍላጎት ቁጠባ ድምር</p>
                        </div>
                    </div>
                    <div class="col-md-8">
                        <div class="card-custom">
                            <h5 class="text-warning fw-bold mb-2"><i class="fa-solid fa-bullhorn"></i> ወቅታዊ የእድር ማስታወቂያዎች</h5>
                            <div id="m-notice-display" class="alert alert-info py-2 small"></div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        async function refreshData() {
            ...
            if(sessionUser && sessionUser.role === 'member') {
                ...
                document.getElementById('m-rules-display').innerText = currentIddir.rules || "ህገ-ደንብ አልተጫነም::";
        ...
        async function adminSaveRules() {
            ...
            await fetch('/api/admin/save-rules', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({ iddir_id: selectedIddirId, fee, dead, banks: [b1, b2], rules })
            });
            alert("🏛️ የእድር የፋይናንስ ህግጋትና ደንብ ተለጥፏል!");
            refreshData();
        }
        ...
        async function adminPostNotice() {
            ...
            await fetch('/api/admin/post-notice', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
...
app.post('/api/super/create-iddir', (req, res) => {
    ...
    masterDB.iddirs[id] = { name, admin_name: admin, admin_phone: phone, admin_pass: pass, monthly_fee: 0, deadline: "-", bank_accounts: [], rules: "", assets: [], notices: [] };
...
app.post('/api/admin/save-rules', (req, res) => {
    const { iddir_id, fee, dead, banks, rules } = req.body;
